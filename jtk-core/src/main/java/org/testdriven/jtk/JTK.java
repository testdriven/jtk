...
public class JTK {
	...
	public JTK(String sourcedir, String classesdir) throws IOException {
		...
		if (!sourcesDir.exists() || !sourcesDir.isDirectory()) {
			throw new IOException("Sources directory "
					+ sourcesDir.getAbsolutePath() + "doesn't exists");
		}

		this.classesDir = new File(classesdir);
		if (!classesDir.exists() || !classesDir.isDirectory()) {
			throw new IOException("Classes directory "
					+ classesDir.getAbsolutePath() + "doesn't exists");
		}
	}

	public TestCaseAnalyzer createTestCaseAnalyzer() {
		AnalisysEngine engine = new JUnit4AnalisysEngine(Arrays
				.asList("org.junit.Assert"));
